...
t0=tic;s(1)=-1;d=0.1;for n=1:N
 s(n+1)=exp(s(n))*cos(s(n)+d);end
 t_dyn=toc(t0);

--_____________________________________

clear s d;
t0=tic;s=zeros([N+1,1]);t_mem=toc(t0);t0=tic;s(1)=-1; d=0.1;for n=1:N
s(n+1)=exp(s(n))*cos(s(n)+d);end
t_loop=toc(t0);
...
clear s d;
t0=tic;s=zeros([N+1,1]);s(1)=-1;d=0.1;for n=1:N
 s(n+1)=exp(s(n))*cos(s(n)+d);end
 t_comb=toc(t0);
disp([ ])
...
clear s d;
 t0=cputime;s(1)=-1;d=0.1;for n=1:N
 s(n+1)=exp(s(n))*cos(s(n)+d);end
 t_dyn=cputime-t0;
...
clear s d;
t0=cputime;s=zeros([N+1,1]);t_mem=cputime-t0;t0=cputime;s(1)=-1;d=0.1;for n=1:N
 s(n+1)=exp(s(n))*cos(s(n)+d);end
 t_loop=cputime-t0;
...
clear s d;
t0=cputime;s=zeros([N+1,1]);s(1)=-1;d=0.1;for n=1:N
 s(n+1)=exp(s(n))*cos(s(n)+d);end
 t_comb=cputime-t0;
disp([ ])
disp("t_dyn, t_mem, t_loop_ t_mem + t_loop, t_comb")
