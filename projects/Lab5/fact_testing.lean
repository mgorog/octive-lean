...
function fact_testing()
    ...
    if compute
        disp("Timing built-in factorial tic;
        for i = 1:N
            factorial(n);
        end
        t_builtin = toc / N;
        ...
        disp("Timing for-loop factorial tic;
        for i = 1:N
            fact_for(n);
        end
        t_for = toc / N;
        ...
        disp("Timing recursive factorial tic;
        for i = 1:N
            fact_recursive(n);
        end
        t_recursive = toc / N;
        ...
        disp("Saving times to vector times = [t_builtin, t_prod, t_for, t_recursive];
